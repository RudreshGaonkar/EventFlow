...
export default function OrganizerStaffTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [sRes, vRes] = await Promise.all([getAllStaff(), getVenues()]);
  ...
  const handleSave = async () => {
    ...
    if (!form.venue_id)
      return showToast('You must assign a venue', 'warning');
    setSaving(true);
    try {
      await addStaff({
  ...
  const venueOptions = venues.map(v => ({
    ...v,
    display_name: `${v.venue_name} — ${v.city_name}`
  }));
