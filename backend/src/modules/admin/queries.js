...
const getAllStates = async () => {
  try {
    ...
    return rows;
  } catch (err) { throw new Error('DB error in getAllStates: ' + err.message); }
};
...
const createState = async (state_name, state_code) => {
  ...
  } catch (err) { throw new Error('DB error in createState: ' + err.message); }
};
...
const updateState = async (state_id, state_name, state_code) => {
  ...
  } catch (err) { throw new Error('DB error in updateState: ' + err.message); }
};
...
const deleteState = async (state_id) => {
  ...
  } catch (err) { throw new Error('DB error in deleteState: ' + err.message); }
};
...
const getCitiesByState = async (state_id) => {
  try {
    ...
    return rows;
  } catch (err) { throw new Error('DB error in getCitiesByState: ' + err.message); }
};
...
const getAllCities = async () => {
  try {
    ...
    return rows;
  } catch (err) { throw new Error('DB error in getAllCities: ' + err.message); }
};
...
const createCity = async (state_id, city_name, city_multiplier) => {
  try {
    ...
    return result.insertId;
  } catch (err) { throw new Error('DB error in createCity: ' + err.message); }
};
...
const updateCity = async (city_id, city_name, city_multiplier) => {
  ...
  } catch (err) { throw new Error('DB error in updateCity: ' + err.message); }
};
...
const updateCityMultiplier = async (city_id, city_multiplier) => {
  ...
  } catch (err) { throw new Error('DB error in updateCityMultiplier: ' + err.message); }
};
...
const deleteCity = async (city_id) => {
  ...
  } catch (err) { throw new Error('DB error in deleteCity: ' + err.message); }
};
...
const getAllVenues = async () => {
  try {
    ...
    return rows;
  } catch (err) { throw new Error('DB error in getAllVenues: ' + err.message); }
};
...
const getVenuesByCity = async (city_id) => {
  try {
    ...
    return rows;
  } catch (err) { throw new Error('DB error in getVenuesByCity: ' + err.message); }
};
...
const createVenue = async (city_id, venue_name, address, total_capacity) => {
  try {
    ...
    return result.insertId;
  } catch (err) { throw new Error('DB error in createVenue: ' + err.message); }
};
...
const updateVenue = async (venue_id, venue_name, address, total_capacity) => {
  ...
  } catch (err) { throw new Error('DB error in updateVenue: ' + err.message); }
};
...
const softDeleteVenue = async (venue_id) => {
  ...
  } catch (err) { throw new Error('DB error in softDeleteVenue: ' + err.message); }
};
...
const getAllUsers = async () => {
  try {
    const pool = getPool();
    const [rows] = await pool.execute(
      'SELECT u.user_id, u.full_name, u.email, u.phone, u.is_active, u.requested_role, r.role_id, r.role_name FROM users u JOIN roles r USING (role_id) ORDER BY u.created_at DESC'
    );
    return rows;
  } catch (err) { throw new Error('DB error in getAllUsers: ' + err.message); }
};
...
const updateUserRole = async (user_id, role_id) => {
  ...
  } catch (err) { throw new Error('DB error in updateUserRole: ' + err.message); }
};
...
const getAllRoles = async () => {
  try {
    ...
    return rows;
  } catch (err) { throw new Error('DB error in getAllRoles: ' + err.message); }
};

// Admin creates a user directly with a chosen role
const insertUser = async (full_name, email, password_hash, phone, role_id) => {
  try {
    const pool = getPool();
    const [result] = await pool.execute(
      'INSERT INTO users (full_name, email, password_hash, phone, role_id) VALUES (?, ?, ?, ?, ?)',
      [full_name, email, password_hash || null, phone || null, role_id]
    );
    return result.insertId;
  } catch (err) { throw new Error('DB error in insertUser: ' + err.message); }
};

const addUserRole = async (user_id, role_id, approved_by) => {
  try {
    const pool = getPool();
    await pool.execute(`
      INSERT INTO user_roles (user_id, role_id, status, approved_at, approved_by)
      VALUES (?, ?, 'Active', NOW(), ?)
      ON DUPLICATE KEY UPDATE
        status = 'Active',
        approved_at = NOW(),
        approved_by = ?
    `, [user_id, role_id, approved_by, approved_by]);
  } catch (err) {
    throw new Error('DB error in addUserRole: ' + err.message);
  }
};
...
const removeUserRole = async (user_id, role_id) => {
  try {
    const pool = getPool();
    await pool.execute(`
      UPDATE user_roles SET status = 'Revoked'
      WHERE user_id = ? AND role_id = ?
    `, [user_id, role_id]);
  } catch (err) {
    throw new Error('DB error in removeUserRole: ' + err.message);
  }
};
