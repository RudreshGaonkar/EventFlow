...
const register = async (req, res) => {
  try {
    ...
    const {
      ...
      college_name,       // only if student
      team_name,          // only if team event
      team_size,          // only if team event
    } = req.body;
    ...
    if (!participant_type) {
      return res.status(400).json({
        success: false,
        message: 'participant_type is required',
      });
    }
    if (participant_type === 'student' && !college_name) {
      return res.status(400).json({
        success: false,
        message: 'college_name is required for student participants',
      });
    }

    // Load event config to check mode + team rules
    ...
    if (config.registration_mode === 'booking') {
      return res.status(400).json({
        success: false,
        message: 'This event uses seat booking, not registration',
      });
    }

    // Team validation at service layer (SP also validates, belt-and-suspenders)
    ...
    if (config.participation_type === 'team' && !team_size) {
      return res.status(400).json({
        success: false,
        message: 'team_size is required for team events',
      });
    }
    ...
    if (out.result_code !== 0) {
      ...
      return res.status(statusMap[out.result_code] || 400).json({
        success: false,
        message: out.result_msg,
      });
    }
    ...
    // Save stripe session id so webhook can match it
    await saveStripeSession(registration_id, session.id);
...
const cancelReg = async (req, res) => {
  try {
    const affected = await cancelRegistration(
...
const handleWebhook = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Only handle registration webhooks (bookings have their own webhook)
    if (session.metadata?.type !== 'registration') {
      return res.sendStatus(200);
    }

    const affected = await confirmPaidRegistration(
      session.id,
